...
def show_day():
    ...
    stop("", ["blue_elephant_seongsu", "musinsa_standard_seongsu",
         ...
         notes=[
             "墨鏡全首爾最大間",
             "韓版 UNIQLO/GU，極簡乾淨風。",
             "球鞋選物，Nike/Adidas/Salomon 少見配色<br>2F 還有 ASSC 等街頭品牌",
             " ",
         ])

    # 4) 點心：Maman Gelato
    stop("", "maman_gelato", others="food",
         notes="Pistachiotella 開心果義式冰淇淋<br>"
               "排隊約 20 min，店內小，可外帶到聖水公園吃")

    # 5) 移動：聖水洞 → 東大門
    note("🚇", "聖水洞 → 東大門",
              "2 號線本線｜4 站｜~10 min｜₩1,500",
              "3 號出口旁邊入口 "
              "→ 2 號線(往시청/홍대입구 方向,外線循環/Outer)<br>"
              "→ 4 站到 東大門歷史文化公園站(동대문역사문화공원역/DDP)<br>"
              "→ 出 14 號出口(14번 출구) → 過清溪川到陳玉華一隻雞")

    # 6) 晚餐：陳玉華一隻雞
    stop("", "jin_okhwa", others="food",
         notes="Tip：先丟馬鈴薯+蒜+蔥下湯，吃完雞肉再加手打麵")

    # 7) 回弘대
    note("🚇", "東大門 → 弘대",
              "2 號線本線直達｜10 站｜~22 min｜₩1,500",
              "「東大門歷史文化公園站 (동대문역사문화공원역 / DDP)」\n\n"
              "→ 14 號入口 → 2 號線(往시청/홍대입구方向,外線循環/Outer)<br>"
              "→ 10 站到「弘大入口站 (홍대입구역 / Hongik Univ.)」<br>"
              "→ 出 9 號出口 (9번 출구) → 走 5 min 回飯店<br>"
